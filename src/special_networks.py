...
class ReachabilityModel(nn.Module):
    ...
    @nn.compact
    def __call__(self, observations, goals, return_logits=False):
        inputs = jnp.concatenate([observations, goals], axis=-1)
        if self.use_layer_norm:
            logits = LayerNormMLP((*self.hidden_dims, 1), activate_final=False, activations=nn.gelu)(inputs)
        else:
            logits = MLP((*self.hidden_dims, 1), activate_final=False, activations=nn.gelu)(inputs)
...
class HierarchicalActorCritic(nn.Module):
    ...
    def reachability(self, observations, goals, low_dim_goals=False, state_rep_grad=True, goal_rep_grad=True, **kwargs):
        state_reps = get_rep(self.encoders['value_state'], targets=observations)
        ...
        return self.networks['reachability'](state_reps, goal_reps, **kwargs)
    ...
    def __call__(self, observations, goals):
        ...
        rets = {
            ...
            'reachability': self.reachability(observations, goals),
        }
        return rets
