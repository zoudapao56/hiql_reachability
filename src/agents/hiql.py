...
from . import iql
from src.special_networks import Representation, HierarchicalActorCritic, RelativeRepresentation, MonolithicVF, ReachabilityModel
...
def compute_reachability_loss(agent, batch, network_params):
    logits = agent.network(
        batch['observations'],
        batch['reachability_goals'],
    ...
    labels = batch['reachability_labels']
    reachability_loss = optax.sigmoid_binary_cross_entropy(logits, labels).mean()
    probs = jax.nn.sigmoid(logits)
    predictions = probs >= agent.config['reachability_threshold']
    accuracy = (predictions == (labels > 0.5)).mean()

    return reachability_loss, {
        'reachability_loss': reachability_loss,
        'reachability_prob': probs.mean(),
        'reachability_pos_prob': jnp.where(labels > 0.5, probs, 0).sum() / jnp.maximum((labels > 0.5).sum(), 1),
        'reachability_neg_prob': jnp.where(labels <= 0.5, probs, 0).sum() / jnp.maximum((labels <= 0.5).sum(), 1),
...
class JointTrainAgent(iql.IQLAgent):
    ...
    def pretrain_update(agent, pretrain_batch, seed=None, value_update=True, actor_update=True, high_actor_update=True, reachability_update=True):
        def loss_fn(network_params):
            ...
            if reachability_update and agent.config['use_reachability']:
                reachability_loss, reachability_info = compute_reachability_loss(agent, pretrain_batch, network_params)
                for k, v in reachability_info.items():
                    info[f'reachability/{k}'] = v
            else:
                reachability_loss = 0.

            loss = value_loss + actor_loss + high_actor_loss + agent.config['reachability_loss_weight'] * reachability_loss
    ...
    def sample_reachable_high_actions(agent,
                                      observations: np.ndarray,
                                      goals: np.ndarray,
                                      *,
                                      seed: PRNGKey,
                                      temperature: float = 1.0,
                                      num_samples: int = None) -> jnp.ndarray:
        num_candidates = agent.config['reachability_resample_attempts']
        dist = agent.network(observations, goals, temperature=temperature, method='high_actor')
        candidates = dist.sample(seed=seed, sample_shape=num_candidates)
        if agent.config['use_rep']:
            candidate_goals = candidates
            low_dim_goals = True
        else:
            candidate_goals = observations[None] + candidates
            low_dim_goals = False
        repeated_observations = jnp.repeat(observations[None], num_candidates, axis=0)
        reachability = agent.network(repeated_observations, candidate_goals, low_dim_goals=low_dim_goals, method='reachability')
        reachable = reachability >= agent.config['reachability_threshold']
        first_reachable = jnp.argmax(reachable)
        best_candidate = jnp.argmax(reachability)
        selected = jnp.where(jnp.any(reachable), first_reachable, best_candidate)
        return candidates[selected]
    sample_reachable_high_actions = jax.jit(sample_reachable_high_actions, static_argnames=('num_samples',))

    def predict_reachability(agent,
                             observations: np.ndarray,
                             goals: np.ndarray,
                             *,
                             low_dim_goals: bool = False) -> jnp.ndarray:
        return agent.network(observations, goals, low_dim_goals=low_dim_goals, method='reachability')
...
def create_learner(
        ...
        rep_type: str = 'state',
        use_waypoints: int = 0,
        use_reachability: int = 0,
        reachability_hidden_dims: Sequence[int] = (256, 256),
        ...
        **kwargs):
        ...
        reachability_def = ReachabilityModel(hidden_dims=reachability_hidden_dims, use_layer_norm=use_layer_norm)
...
def get_default_config():
    config = ml_collections.ConfigDict({
        ...
        'use_reachability': 0,
        'reachability_hidden_dims': (256, 256),
